...
-- Function to get all body parts of the target
...
-- Function to draw a box around the entire target based on the DetectionZone
local function drawBoxAroundTarget(character)
    local parts = getBodyPartsPosition(character)
    local minX, minY, minZ = math.huge, math.huge, math.huge
    local maxX, maxY, maxZ = -math.huge, -math.huge, -math.huge

    -- Get the bounding box for the target based on the body parts
    ...
    -- Get the dimensions of the box to draw based on the DetectionZone settings
    local detectionZone = shared.Forbidden.TriggerBot.Settings.DetectionZone
    local sizeX = (maxX - minX) * detectionZone.x
    local sizeY = (maxY - minY) * detectionZone.y
    local sizeZ = (maxZ - minZ) * detectionZone.z
    ...
    -- Convert the center to screen space
    local screenPos, onScreen = Camera:WorldToViewportPoint(Vector3.new(centerX, centerY, centerZ))
    if onScreen then
        -- Draw a box around the target (this will be a simple rectangular box representation)
        local boxWidth = sizeX * Camera.ViewportSize.X / Camera.FieldOfView
        local boxHeight = sizeY * Camera.ViewportSize.Y / Camera.FieldOfView
        -- Box drawing logic here (could be a UI element or an overlay)
        -- You could implement a visual representation of this box for debugging
    end

    return screenPos, sizeX, sizeY, sizeZ
end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                local screenPos, sizeX, sizeY, sizeZ = drawBoxAroundTarget(TargetPlayer.Character)

                -- Now that we have the box coordinates, check if the mouse is within that box
                local mouseX, mouseY = getMousePosition()
                local targetBoxX1 = screenPos.X - sizeX / 2
                local targetBoxX2 = screenPos.X + sizeX / 2
                local targetBoxY1 = screenPos.Y - sizeY / 2
                local targetBoxY2 = screenPos.Y + sizeY / 2

                -- Check if the mouse is within the box (inside target's bounding area)
                if mouseX >= targetBoxX1 and mouseX <= targetBoxX2 and mouseY >= targetBoxY1 and mouseY <= targetBoxY2 then
                        ...
                        if tool and tool:IsA("Tool") then
                            if not isIgnoringKnife() then
                                ...
                                if shootFunction and shootFunction:IsA("RemoteEvent") then
                                    ...
                                    -- Fire at any pixel within the bounding box of the target
